    ...
    <div class="main-div">
        <form method="POST" style="margin:5px" action="/E-Cshop/Role/edit/id/3.html">
            ...
            <p>为该角色修改分配的权限：
                ...
                <?php if(is_array($data1)): $i = 0; $__LIST__ = $data1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i; echo str_repeat('-',$obj['level']*8);?>
                    <input type="checkbox" name="auth_id[]" style="margin:5px" 
                    value="<?php echo ($obj["id"]); ?>" level="<?php echo ($obj["level"]); ?>"
                    <?php if(strpos(','.$data['auth_id'].',',','.$obj['id'].',')!==false) echo "checked" ?>
                    ><?php echo ($obj["auth_name"]); ?><br/><?php endforeach; endif; else: echo "" ;endif; ?>
                </div>
            </p>
